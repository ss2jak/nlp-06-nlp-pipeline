...
# ============================================================
def length_scatter_plot(
    tokens: list[str],
    output_path: Path,
    title: str,
    LOG: logging.Logger,
) -> None:
    """Plot a scatter plot of token length vs frequency."""

    counts = Counter(tokens)

    # Selecting unique tokens and their frequencies for the scatter plot
    unique_tokens = list(counts.keys())
    frequencies = [counts[token] for token in unique_tokens]
...
def run_analyze(
    ...
    # ============================================================
    # Phase 4.4: Scatter Plot of Token Length vs Frequency
    # ============================================================
    LOG.info("========================")
    LOG.info("PHASE 4.4: Token length vs frequency scatter plot")
    LOG.info("========================")

    length_scatter_plot(
        tokens=tokens,
        output_path=output_dir / "jak_scatter_plot.png",
        title=f"Unique Token Frequency vs Length: {title[:40]}...",
        LOG=LOG,
    )
